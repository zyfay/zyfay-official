...
function GamesSlider({ filtered, page, setPage, totalPages }) {
  ...
  function onTouchEnd() {
    setDragging(false);
    const threshold = 50;
    if (touchDeltaX.current < -threshold && page < totalPages - 1) setPage(p => p + 1);
    else if (touchDeltaX.current > threshold && page > 0) setPage(p => p - 1);
    touchDeltaX.current = 0;
    setDragOffset(0);
  }
      ...
      >
        ...
        >
          {[...Array(totalPages)].map((_, pIdx) => (
            <div key={pIdx} className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-4 flex-shrink-0 w-full pr-0.5">
              {filtered.slice(pIdx * PAGE_SIZE, pIdx * PAGE_SIZE + PAGE_SIZE).map((game) => (
