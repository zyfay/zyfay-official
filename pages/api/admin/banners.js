...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    const { title, subtitle, link_url, button_label, is_active, sort_order, image_base64, image_name } = req.body;

    const image_url = await uploadImage(image_base64, image_name);
    if (!image_url) return res.status(400).json({ success: false, message: 'Gambar wajib diupload' });

    const { data, error } = await supabaseAdmin
      .from('banners')
      .insert({
        title, subtitle, link_url, button_label: button_label || 'MULAI',
