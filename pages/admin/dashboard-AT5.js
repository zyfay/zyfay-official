...
import {
  ...
  CheckCircle, XCircle, AlertCircle, Search, Eye, EyeOff, Zap, Wrench, Flame, Bell, Megaphone, CalendarClock, Images, ArrowLeft, Key, Code, Award, Link as LinkIcon, ZoomIn
} from 'lucide-react';
...
function ProductModal({ product, games, onClose, onSaved }) {
  ...
  useEffect(() => {
    fetch('/api/admin/product-categories', { credentials: 'include' })
      .then(r => r.json())
      .then(d => setShopCategories(d.categories || []));
  }, []);
  ...
  async function handleAddCategory() {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setShopCategories(c => [...c, data.category]);
        setForm(f => ({ ...f, shop_category_id: data.category.id }));
        setNewCatName('');
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="col-span-2">
              <label className="block text-sm font-medium mb-1">
                Kategori Produk Lainnya <span className="text-muted font-normal">(buat filter di halaman "Produk Zyfay Lainnya")</span>
              </label>
              <select value={form.shop_category_id} onChange={e => setForm(f => ({ ...f, shop_category_id: e.target.value }))} className="input-field mb-2">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="Kategori baru, mis. Akun Mobile Legends"
