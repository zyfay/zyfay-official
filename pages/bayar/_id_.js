...
export default function BayarPage() {
  ...
  async function load() {
    try {
      ...
      if (o.payment_data?.payment_number) {
          ...
          {['failed', 'expired'].includes(order.order_status) ? (
            ...
            <div className="card p-6 mb-5 text-center space-y-4">
              {paymentData.payment_method === 'qris' ? (
                <>
                  <img
                    src={`https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(paymentData.payment_number)}`}
                    alt="QRIS"
              ...
              <div>
                <div className="text-muted text-xs mb-1">
                  {paymentData.payment_method === 'qris' ? 'Salin kode QR' : 'Salin nomor Virtual Account'}
                </div>
                <div className="flex items-center gap-2 bg-card rounded-xl p-3 border border-border">
                  <span className="font-mono text-xs text-muted flex-1 truncate">
                    {paymentData.payment_method === 'qris'
                      ? `${paymentData.payment_number.slice(0, 30)}...`
                      : paymentData.payment_number}
                  </span>
                  <button onClick={() => copyToClipboard(paymentData.payment_number)} className="text-muted hover:text-white flex-shrink-0">
              ...
              >
                {regenerating
                  ? <><Loader2 size={14} className="animate-spin" /> Membuat ulang...</>
                  : <><RefreshCw size={14} /> Kedaluwarsa? Buat Ulang</>
                }
              </button>
            </div>
          ) : (
            <div className="card p-6 mb-5 text-center">
              ...
              <p className="text-muted text-sm mb-4">Pembayaran belum tersedia untuk pesanan ini.</p>
              <button onClick={() => regenerate(false)} disabled={regenerating} className="btn-primary text-sm">
                {regenerating
                  ...
                  : <><RefreshCw size={14} /> Buat Pembayaran</>
                }
              </button>
            </div>
          )}
