...
import { Search, Package, ArrowLeft, Loader2, CheckCircle, Clock, XCircle, RefreshCw, CreditCard, History } from 'lucide-react';
    ...
    <>
      ...
      <div className="min-h-screen bg-bg">
        ...
        <div className="max-w-lg mx-auto px-4 py-10">
          ...
          <Link href="/riwayat" className="card p-3 mb-6 flex items-center gap-3 hover:border-primary/40 transition-colors">
            <div className="w-9 h-9 bg-primary/15 rounded-lg flex items-center justify-center flex-shrink-0">
              <History size={16} className="text-primary-glow" />
            </div>
            <div className="flex-1 min-w-0">
              <div className="font-medium text-sm">Lupa ID pesanan?</div>
              <div className="text-muted text-xs">Lihat riwayat transaksi di perangkat ini</div>
            </div>
          </Link>
              ...
              <div className="card overflow-hidden">
                ...
                <div className="p-4 space-y-3">
                  {[
                    ...
                    order.form_data?.user_id && { l: 'ID Game', v: order.form_data.user_id },
                    order.game_nickname && { l: 'Nickname', v: order.game_nickname },
                    { l: 'Total', v: `Rp ${order.product_price?.toLocaleString('id-ID')}` },
                    order.voucher_code && { l: 'Voucher', v: order.voucher_code },
                ...
                {order.order_status === 'pending' && (
                  <div className="p-4 border-t border-border">
                    <Link href={`/bayar/${order.id}`} className="btn-primary w-full text-sm">
