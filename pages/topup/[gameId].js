...
import { ArrowLeft, CheckCircle, Loader2, CreditCard, ShieldCheck, Flame, Clock, Search, AlertCircle, Tag, X } from 'lucide-react';
...
export default function TopUpPage() {
  ...
  const [paymentCategory, setPaymentCategory] = useState('qris'); // 'qris' | 'va'
  const [checkingNickname, setCheckingNickname] = useState(false);
  const [nicknameResult, setNicknameResult] = useState(null); // { name } | { error }
  const [voucherCode, setVoucherCode] = useState('');
  const [voucherApplied, setVoucherApplied] = useState(null); // { code, discount_percent, discount_amount, final_amount }
  ...
  async function fetchData() {
    ...
    // Kalau datang dari link produk spesifik (mis. kartu ZySale), langsung
    // pilih produknya & lompat ke step "Data Akun", gak perlu milih nominal lagi.
    const preselectId = router.query.product;
    if (preselectId) {
      const match = (prodRes.data || []).find(p => p.id === preselectId);
      if (match) {
        setSelected(match);
        setStep(2);
      }
  ...
  function validateStep2() {
    for (const f of (game?.fields || [])) {
      if (!form[f.name]?.trim()) {
        toast.error(`${f.label} wajib diisi`, { id: 'form-validation' });
        return false;
      }
    }
    if (!email?.trim()) {
      toast.error('Email wajib diisi', { id: 'form-validation' });
      return false;
    }
    // validasi format email
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      toast.error('Format email tidak valid', { id: 'form-validation' });
      return false;
    }
    return true;
  }

  const nicknameConfig = getNicknameGameConfig(gameId);

  async function handleCheckNickname() {
    const userIdField = (game?.fields || []).find(f => f.name === 'user_id') || (game?.fields || [])[0];
    const serverField = (game?.fields || []).find(f => ['zone_id', 'server_id', 'server'].includes(f.name));
    const userId = userIdField ? form[userIdField.name] : '';
    const serverId = serverField ? form[serverField.name] : '';

    if (!userId?.trim()) {
      toast.error(`${userIdField?.label || 'User ID'} wajib diisi dulu`, { id: 'nickname-check' });
      return;
    }
    if (nicknameConfig?.needsServer && !serverId?.trim()) {
      toast.error(`${serverField?.label || 'Server ID'} wajib diisi dulu`, { id: 'nickname-check' });
      return;
    }
    ...
    try {
      const res = await fetch('/api/cekid-game', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ game_id: gameId, user_id: userId, server_id: serverId }),
      });
      const data = await res.json();
      if (data.success) {
        setNicknameResult({ name: data.name });
      } else {
        setNicknameResult({ error: data.message || 'ID tidak ditemukan' });
      }
    } catch (e) {
      setNicknameResult({ error: 'Gagal memeriksa ID, coba lagi' });
    } finally {
  ...
  async function applyVoucher() {
    ...
    const gameUserId = (game?.fields || []).find(f => f.name === 'user_id') ? form['user_id'] : Object.values(form)[0];
    if (!gameUserId?.trim()) {
      toast.error('Isi User ID dulu sebelum pakai voucher', { id: 'voucher' });
      return;
    }
    ...
    try {
      const res = await fetch('/api/vouchers/validate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ code: voucherCode.trim(), game_user_id: gameUserId, amount: effectivePrice(selected) }),
      });
      const data = await res.json();
      if (data.success) {
        setVoucherApplied(data);
        toast.success(`Voucher diterapkan, hemat Rp ${data.discount_amount.toLocaleString('id-ID')}`);
      } else {
  ...
  function totalToPay() {
    return voucherApplied ? voucherApplied.final_amount : effectivePrice(selected);
  }
    ...
    try {
      const res = await fetch('/api/orders/create', {
        ...
        body: JSON.stringify({
          ...
          voucher_code: voucherApplied ? voucherApplied.code : null,
          game_nickname: nicknameResult?.name || null,
        }),
      });
      ...
      addToHistory({
        ...
        game_name: game.name,
        product_name: selected.name,
        product_price: data.order.product_price,
        game_nickname: nicknameResult?.name || null,
      ...
      toast.error(e.message || 'Gagal membuat pesanan', { id: 'checkout-error' });
    } finally {
      setSubmitting(false);
    }
  }
        ...
        <div className="max-w-2xl mx-auto px-4 py-8">
          ...
          {step === 1 && (
            <div>
              ...
              <button
                ref={lanjutRef}
                onClick={() => selected ? setStep(2) : toast.error('Pilih nominal terlebih dahulu', { id: 'form-validation' })}
          ...
          {step === 2 && (
            <div>
              ...
              <div className="space-y-4 mb-6">
                  ...
                  <div key={f.name}>
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        value={form[f.name] || ''}
                        onChange={e => { setForm({ ...form, [f.name]: e.target.value }); setNicknameResult(null); }}
                        className="input-field flex-1"
                      />
                      {nicknameConfig && f.name === 'user_id' && (
                        <button
                          type="button"
                          onClick={handleCheckNickname}
                          disabled={checkingNickname}
                          className="btn-secondary text-sm px-4 flex-shrink-0 whitespace-nowrap"
                        >
                          {checkingNickname
                            ? <Loader2 size={14} className="animate-spin" />
                            : <><Search size={14} /> Cek ID</>
                          }
                        </button>
                      )}
                    </div>
                ...
                {nicknameResult?.name && (
                  <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-3 flex items-center gap-2.5">
                    <CheckCircle size={16} className="text-emerald-400 flex-shrink-0" />
                    <div className="text-sm">
                      <span className="text-muted">Nama akun: </span>
                      <span className="font-semibold text-emerald-400">{nicknameResult.name}</span>
                    </div>
                  </div>
                )}
                {nicknameResult?.error && (
                  <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-3 flex items-center gap-2.5">
                    <AlertCircle size={16} className="text-red-400 flex-shrink-0" />
                    <div className="text-sm text-red-400">{nicknameResult.error}</div>
                  </div>
                )}
                ...
                <div>
                  <label className="block text-sm font-medium mb-1.5">No HP (opsional, buat notifikasi WhatsApp)</label>
                  <input
                    type="text"
          ...
          {step === 3 && (
            <div>
              ...
              <div className="card p-4 mb-5 space-y-2.5">
                ...
                {voucherApplied && (
                  <div className="flex justify-between text-xs text-emerald-400">
                    <span>Potongan Voucher {voucherApplied.code}</span>
                    <span>Rp {voucherApplied.discount_amount.toLocaleString('id-ID')}</span>
                  </div>
                )}
              </div>
              ...
              {/* Voucher */}
              <div className="mb-5">
                <div className="font-semibold text-sm mb-2.5 flex items-center gap-1.5"><Tag size={14} /> Kode Voucher</div>
                {voucherApplied ? (
                  <div className="card p-3 border-emerald-500/40 bg-emerald-500/5 flex items-center justify-between">
                    <div>
                      <div className="font-semibold text-sm text-emerald-400">{voucherApplied.code}</div>
                      <div className="text-muted text-xs">
                        Diskon {voucherApplied.discount_percent}% · Hemat Rp {voucherApplied.discount_amount.toLocaleString('id-ID')}
                      </div>
                    </div>
                    <button onClick={removeVoucher} className="text-muted hover:text-red-400 p-1.5">
                      <X size={16} />
                    </button>
                  </div>
                ) : (
                  <div>
                    <div className="flex gap-2">
                      <input
                        type="text"
                        placeholder="Contoh: ZYWARSEPTEMBER"
                        value={voucherCode}
                        onChange={e => { setVoucherCode(e.target.value.toUpperCase()); setVoucherError(''); }}
                        onKeyDown={e => e.key === 'Enter' && applyVoucher()}
                        className="input-field flex-1 uppercase"
                      />
                      <button onClick={applyVoucher} disabled={voucherChecking || !voucherCode.trim()} className="btn-secondary px-4 flex-shrink-0 text-sm">
                        {voucherChecking ? <Loader2 size={14} className="animate-spin" /> : 'Terapkan'}
                      </button>
                    </div>
              ...
              <div className="mb-5">
                ...
                {/* Tab QRIS / Virtual Account */}
                <div className="grid grid-cols-2 gap-2 bg-card-hover p-1 rounded-xl mb-3">
                  <button
                    onClick={() => { setPaymentCategory('qris'); setPaymentMethod('qris'); }}
                    className={`py-2.5 rounded-lg text-sm font-semibold transition-all active:scale-95 ${
                      paymentCategory === 'qris' ? 'bg-primary text-white shadow-glow' : 'text-muted hover:text-white'
                    }`}
                  >
                    QRIS
                  </button>
                  <button
                    onClick={() => {
                      ...
                      const firstVa = PAYMENT_METHODS.find(m => m.type === 'va');
                      if (getPaymentMethod(paymentMethod).type !== 'va') setPaymentMethod(firstVa.code);
                    }}
                    className={`py-2.5 rounded-lg text-sm font-semibold transition-all active:scale-95 ${
                      paymentCategory === 'va' ? 'bg-primary text-white shadow-glow' : 'text-muted hover:text-white'
                    }`}
                  >
                    Virtual Account
                  </button>
                ...
                {paymentCategory === 'qris' ? (
                  <div className="card p-4 flex items-center gap-3 border-primary bg-primary/10">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {PAYMENT_METHODS.filter(m => m.type === 'va').map(m => (
                      <button
                        ...
                        className={`relative card p-3 flex items-center gap-2.5 text-left transition-all active:scale-95 ${
                          paymentMethod === m.code ? 'border-primary bg-primary/10' : 'hover:border-primary/40'
                        }`}
                      >
                        {paymentMethod === m.code && (
                          <CheckCircle size={14} className="absolute top-1.5 right-1.5 text-primary-glow" />
                        )}
              ...
              <div className="flex gap-3">
                <button onClick={() => setStep(2)} className="btn-secondary flex-1 text-sm py-2.5 active:scale-95 transition-transform">
                  Kembali
                </button>
                <button onClick={handleCheckout} disabled={submitting} className="btn-primary flex-1 text-sm py-2.5 active:scale-95 transition-transform">
                  {submitting
