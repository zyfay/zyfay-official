...
import {
  ...
  BadgeDollarSign, Star, Ticket, Gamepad2, Clock, PackageSearch, ImageIcon, Award, ExternalLink,
} from 'lucide-react';
...
export default function Home() {
  ...
  async function fetchOtherProducts() {
    try {
      ...
      const data = await res.json();
      setOtherProducts((data.products || []).slice(0, 6));
    } catch (e) { console.error(e); }
  }
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 mt-16">
      <div className="text-center mb-6">
        <div className="inline-flex items-center gap-2 text-muted text-xs uppercase tracking-widest font-semibold">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {sponsors.map(s => {
          const CardTag = s.link_url ? 'a' : 'div';
          const cardProps = s.link_url
          ...
          return (
            <CardTag
              key={s.id}
              {...cardProps}
              className={`card p-5 flex items-start gap-4 transition-all ${s.link_url ? 'hover:border-primary/40 hover:-translate-y-0.5' : ''}`}
            >
              <div className="w-14 h-14 rounded-2xl bg-card-hover border border-border flex items-center justify-center flex-shrink-0 overflow-hidden">
                {s.image_url
                  ? <img src={s.image_url} alt={s.title} className="w-full h-full object-cover" />
                  : <Award size={24} className="text-primary-glow" />}
              </div>
              <div className="flex-1 min-w-0">
                <div className="font-display font-bold text-white truncate">{s.title}</div>
                {s.description && (
                  <p className="text-muted text-sm mt-1 line-clamp-2">{s.description}</p>
                )}
                {s.link_url && (
                  <div className="flex items-center gap-1 text-primary-glow text-xs font-medium mt-2">
...
function FlashSaleCard({ sale }) {
  ...
  const href = p.game_id ? `/topup/${p.game_id}?product=${p.id}` : '/produk-lainnya';

  return (
    <Link href={href} className="card-hover rounded-xl overflow-hidden relative flex items-center gap-2.5 p-2">
      {discount > 0 && (
        <div className="absolute top-1 left-1 bg-red-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full z-10">
          -{discount}%
        </div>
      )}
      <div className="w-14 h-14 flex-shrink-0 bg-card-hover rounded-lg flex items-center justify-center overflow-hidden">
        {sale.image_url
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-muted/60 text-[10px] line-through">Rp {p.price.toLocaleString('id-ID')}</span>
        </div>
